...
namespace SC.XR.Unity.Module_InputSystem.InputDeviceHand
{
    public class EffectHandModel : AbstractHandModel {
        ...
        public override void OnSCAwake() {
            ...
            if (API_Module_SDKConfiguration.HasKey("Module_InputSystem", "HandRenderEMISSIONIntensity")) {
                try {
                    HandRenderEMISSIONIntensity = API_Module_SDKConfiguration.GetFloat("Module_InputSystem", "HandRenderEMISSIONIntensity", 0);
                    DebugMy.Log("HandRenderEMISSIONIntensity From SDKConfig:" + HandRenderEMISSIONIntensity, this, true);
                } catch (Exception e) {
                    Debug.Log(e);
                }
            }
        ...
        protected virtual void OnEnable() {
            HandRenderSet(ShowHandRender);
            //mEffectCoroutine = StartCoroutine(TriggerEffect(EffectDuration));
            //if (EffectDuration <= 0) {
            //    mRenderer.sharedMaterial = mNomalMaterialNoAlpha;
            //} else if (!EffectFinish && mEffectMaterial && mRenderer && mNomalMaterial && mNomalMaterialNoAlpha) {
            //    mEffectCoroutine = StartCoroutine(TriggerEffect(EffectDuration));
            //}
        }

        public override void HandRenderSet(bool isOn) {
            if (isOn == false) {
                mRenderer.enabled = false;
            } else {
                mRenderer.material = mNomalMaterialNoAlpha;
                mRenderer.material.EnableKeyword("_EMISSION");
                mRenderer.material.SetColor("_EmissionColor", new Vector4(HandRenderEMISSIONIntensity, HandRenderEMISSIONIntensity, HandRenderEMISSIONIntensity, 0));
        ...
        IEnumerator TriggerEffect(float durationTime) {
            ...
            //    mRenderer.sharedMaterial = mNomalMaterialNoAlpha;
            //}
            mNomalMaterialNoAlpha.DisableKeyword("_EMISSION");
            if (!IsTransparent) {
                time = 0;
                mNomalMaterialNoAlpha.color = new Color(51,49,49,0);
                mNomalMaterialNoAlpha.SetFloat("_Metallic", 0);
                // mRenderer.sharedMaterial = mNomalMaterial;
                while (time <= 3) {
                    time += Time.deltaTime;
                    //  mNomalMaterial.SetFloat("_Alpha", time);
                    mNomalMaterialNoAlpha.SetFloat("_Metallic", time/3);
                    yield return null;
                }
               // mRenderer.sharedMaterial = mNomalMaterialNoAlpha;
            }
            mNomalMaterialNoAlpha.EnableKeyword("_EMISSION");
