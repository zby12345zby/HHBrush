...
namespace SC.XR.Unity.Module_InputSystem.InputDeviceGC.BT3Dof {
    public class InputDataGetBT3DofPosture : InputDataGetGCPosture {
        ...
        protected override void UpdatePosition() {
            ...
            if (inputDataGetBT3Dof.inputDeviceBT3DofPart.PartType == InputDevicePartType.GCOne) {
                if (API_GSXR_Slam.SlamManager != null && API_GSXR_Slam.SlamManager.gameObject.activeSelf) {
                    inputDataGetBT3Dof.inputDeviceBT3DofPart.inputDataBT3Dof.position = API_GSXR_Slam.SlamManager.head.position + device1PositionDeltaWithHead;
                }
            } else if (inputDataGetBT3Dof.inputDeviceBT3DofPart.PartType == InputDevicePartType.GCTwo) {
                if (API_GSXR_Slam.SlamManager != null && API_GSXR_Slam.SlamManager.gameObject.activeSelf) {
                    inputDataGetBT3Dof.inputDeviceBT3DofPart.inputDataBT3Dof.position = API_GSXR_Slam.SlamManager.head.position + device2PositionDeltaWithHead;
                }
            } else {
        ...
        protected override void UpdateRotation() {
            ...
            if (Application.platform != RuntimePlatform.Android) {
                if (API_GSXR_Slam.SlamManager != null && API_GSXR_Slam.SlamManager.gameObject.activeSelf) {
                    inputDataGetBT3Dof.inputDeviceBT3DofPart.inputDataBT3Dof.rotation = API_GSXR_Slam.SlamManager.head.rotation;
                }
                return;
            }
