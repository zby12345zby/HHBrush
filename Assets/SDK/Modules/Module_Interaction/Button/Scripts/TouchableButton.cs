...
public class TouchableButton : PokeHandler {
    ...
    public bool enforceFrontPush = true;
    ...
    private bool HasPassedThroughStartPlane(Vector3 positionWorldSpace) {
        ...
        //<= beacause the value is negative
        return distanceAlongPushDirection <= (startPushDistance * 0.2f);
    }
    ...
    public override void OnPokeUpdated(TouchPointer touchPointer, SCPointEventData eventData) {
        ...
        if(IsTouching) {
            currentPushDistance = Mathf.Lerp(currentPushDistance, GetFarthestDistanceAlongPressDirection(touchPointer.ToucherPosition), 0.5f);
