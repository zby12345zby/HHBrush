...
public class GameControllerDevicePartManipulation : AbstractDevicePartManipulation
{
    ...
    private Quaternion localRotationInGameController;
    private Transform oneControllerTransform;
    ...
    public override void OneDevicePartInit(ManipulationHandler manipulationHandler, Dictionary<InputDevicePartType, SCPointEventData> eventDataDic, Transform targetTransform, MoveLogic moveLogic, RotateLogic rotateLogic, ScaleLogic scaleLogic)
    {
        ...
        oneControllerTransform = this.eventData.inputDevicePartBase.inputDevicePartUIBase.modelBase.transform;
        startRotation = targetTransform.rotation;
        localRotationInGameController = Quaternion.Inverse(oneControllerTransform.rotation) * startRotation;
    ...
    {
        pointerPose.position = this.eventData.Position3D;
        pointerPose.rotation = startRotation;

        Quaternion rotation = targetTransform.rotation;
        if (manipulationHandler.oneGameControllerRotate)
        {
            rotation = oneControllerTransform.rotation * localRotationInGameController;
        }
