...
public class BoundingBox : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler
{
    ...
    [SerializeField]
    private BoundingBoxAssets m_HandlerAssets;
    public BoundingBoxAssets BoundingBoxAssets {
        get {
            if (m_HandlerAssets == null) {
                m_HandlerAssets = Resources.Load<BoundingBoxAssets>("HandlerAssets/DefaultAssets");
            }
            return m_HandlerAssets;
        }
    }

    //[SerializeField]
    //[AssetPreAssign("Assets/SDK/Common/StandardAssets/Materials/BoundingBox.mat", typeof(Material))]
    private Material m_boxFocusDisplayMat;
    public Material boxFocusDisplayMat {
        get {
            if (m_boxFocusDisplayMat == null) {
                if (BoundingBoxAssets) {
                    m_boxFocusDisplayMat = BoundingBoxAssets.boxFocusDisplayMat;
                }
            }
            return m_boxFocusDisplayMat;
        }
    }

    //[SerializeField]
    //[AssetPreAssign("Assets/SDK/Common/StandardAssets/Materials/BoundingBoxGrabbed.mat", typeof(Material))]
    private Material m_boxGrabDisplayMat;
    public Material boxGrabDisplayMat {
        get {
            if (m_boxGrabDisplayMat == null) {
                if (BoundingBoxAssets) {
                    m_boxGrabDisplayMat = BoundingBoxAssets.boxGrabDisplayMat;
                }
            }
            return m_boxGrabDisplayMat;
        }
    }
    ...
    public Material HandleMaterial {
        get {
            if (m_HandleMaterial == null) {
                if (BoundingBoxAssets) {
                    m_HandleMaterial = BoundingBoxAssets.HandleMaterial;
                }
            }
    ...
    private Material m_HandleGrabMaterial;
    public Material HandleGrabMaterial {
        get {
            if (m_HandleGrabMaterial == null) {
                if (BoundingBoxAssets) {
                    m_HandleGrabMaterial = BoundingBoxAssets.HandleGrabMaterial;
                }
            }
            return m_HandleGrabMaterial;
        }
    }

    //[SerializeField]
    //[Header("Scale Handles")]
    //[AssetPreAssign("Assets/SDK/Common/StandardAssets/Prefabs/BoundingBox_ScaleHandle.prefab", typeof(GameObject))]
    private GameObject m_CornerPrefab;
    public GameObject CornerPrefab {
        get {
            if (m_CornerPrefab == null) {
                if (BoundingBoxAssets) {
                    m_CornerPrefab = BoundingBoxAssets.CornerPrefab;
                }
            }
    ...
    //[SerializeField]
    //[AssetPreAssign("Assets/SDK/Common/StandardAssets/Prefabs/BoundingBox_ScaleHandle_Slate.prefab", typeof(GameObject))]
    private GameObject m_CornerSlatePrefab;
    public GameObject CornerSlatePrefab {
        get {
            if (m_CornerSlatePrefab == null) {
                if (BoundingBoxAssets) {
                    m_CornerSlatePrefab = BoundingBoxAssets.CornerSlatePrefab;
                }
            }
            return m_CornerSlatePrefab;
        }
    }


    public bool activeScaleMinRestrict = true;
    [Tooltip("Minimum scaling allowed relative to the world size Unit of m")]
    public float scaleMinimum = 0.05f;

    public bool activeScaleMaxRestrict = false;
    [Tooltip("Maximum scaling allowed relative to the world size Unit of m")]
    public float scaleMaximum = 2.0f;

    public Vector3 boundingBoxInitScale { get; private set; }
    public Vector3 calculateMinScale {
        get {
            Vector3 temp = (CurrentBoundsExtents * 2 / scaleMinimum);
            return new Vector3(boundingBoxInitScale.x / temp.x, boundingBoxInitScale.y / temp.y, boundingBoxInitScale.z / temp.z);
        }
    }
    public Vector3 calculateMaxScale {
        get {
            Vector3 temp = (CurrentBoundsExtents * 2 / scaleMaximum);
            return new Vector3(boundingBoxInitScale.x / temp.x, boundingBoxInitScale.y / temp.y, boundingBoxInitScale.z / temp.z);
        }
    }
    ...
    //[AssetPreAssign("Assets/SDK/Common/StandardAssets/Prefabs/BoundingBox_RotateHandle.prefab", typeof(GameObject))]
    private GameObject m_SidePrefab;
    public GameObject SidePrefab {
        get {
            if (m_SidePrefab == null) {
                if (BoundingBoxAssets) {
                    m_SidePrefab = BoundingBoxAssets.SidePrefab;
                }
            }
            return m_SidePrefab;
        }
    ...
    public GameObject facePrefab {
        get {
            if (m_facePrefab == null) {
                if (BoundingBoxAssets) {
                    m_facePrefab = BoundingBoxAssets.facePrefab;
                }
            }
            return m_facePrefab;
        }
    ...
    private void RecaculateBounds()
    {
        ...
        boundingBoxInitScale = BoundBoxCollider.transform.lossyScale;
