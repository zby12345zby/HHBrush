...
public class SideBoundingBoxRoot : AbstractBoundingBoxRoot
{
    ...
    public override void OnHandlerDrag(PointerEventData eventData)
    {
        ...
        if (boundingBox.FlattenAxis == BoundingBox.FlattenModeType.DoNotFlatten) {
    ...
    private void ReCreateVisual()
    {
        ...
        for (int i = 0; i < SIDE_NUM; i++)
        {
            ...
            if (this.boundingBox.FlattenAxis == BoundingBox.FlattenModeType.DoNotFlatten) {
                invScale = Mathf.Max(Mathf.Max(boundingBox.CurrentBoundsExtents.x, boundingBox.CurrentBoundsExtents.y), boundingBox.CurrentBoundsExtents.z) / 5;
            } else {
                invScale = Mathf.Max(Mathf.Max(boundingBox.CurrentBoundsExtents.x, boundingBox.CurrentBoundsExtents.y), boundingBox.CurrentBoundsExtents.z) / 10;
            }

            handles[i].visual.transform.localScale = new Vector3(invScale, invScale, invScale);
            handles[i].visual.transform.SetParent(handles[i].root.transform, false);
