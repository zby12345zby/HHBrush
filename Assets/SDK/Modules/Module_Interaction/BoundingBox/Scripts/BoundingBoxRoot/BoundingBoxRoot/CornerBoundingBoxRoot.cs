...
public class CornerBoundingBoxRoot : AbstractBoundingBoxRoot
{
    ...
    public override void OnHandlerDrag(PointerEventData eventData)
    {
        ...
        Vector3 caculatewordScale = TransformSize(boundingBox.transform, newScale);
        //Debug.Log(boundingBox.calculateMinScale+" "+boundingBox.calculateMaxScale+" "+ caculatewordScale);
        if (
            (
            boundingBox.activeScaleMinRestrict &&
            ( boundingBox.calculateMinScale.x > caculatewordScale.x 
            && boundingBox.calculateMinScale.y > caculatewordScale.y
            && boundingBox.calculateMinScale.z > caculatewordScale.z)

            ||

            boundingBox.activeScaleMaxRestrict &&
            (boundingBox.calculateMaxScale.x < caculatewordScale.x
            || boundingBox.calculateMaxScale.y < caculatewordScale.y
            || boundingBox.calculateMaxScale.z < caculatewordScale.z)
    ...
    private Vector3 TransformSize(Transform transform, Vector3 localSize) {
    ...
    private void ReCreateVisual()
    {
        ...
        for (int i = 0; i < CORNER_NUM; i++)
        {
            ...
            {
                handles[i].visual = GameObject.Instantiate(prefabToInstantiate);
            }

            float invScale;
            if (this.boundingBox.FlattenAxis == BoundingBox.FlattenModeType.DoNotFlatten) {
                invScale = Mathf.Max(Mathf.Max(currentBoundsExtents.x, currentBoundsExtents.y), currentBoundsExtents.z) / 5;
            } else {
                invScale = Mathf.Max(Mathf.Max(currentBoundsExtents.x, currentBoundsExtents.y), currentBoundsExtents.z) / 10;
            }

            handles[i].visual.transform.localScale = new Vector3(invScale, invScale, invScale);

            handles[i].visual.transform.SetParent(handles[i].visualsScale,false);
            ...
            collider.size = Vector3.one * Mathf.Max(Mathf.Max(handles[i].bounds.size.x, handles[i].bounds.size.y), handles[i].bounds.size.z) * invScale * 2f;
            collider.center = Vector3.zero;// handles[i].bounds.center * invScale;
