...
namespace TiltBrush
{
    ...
    { (int)KeyboardShortcut.InstantUpload, new KeyCode[] { KeyCode.U } },
        ...
        public bool GetCommand(SketchCommands rCommand)
        {
            ...
            //Debug.Log("当前状态为：" + API_Module_InputSystem_KS.KSRight.inputDataKS.inputKeys.GetKeyCurrentState(InputKeyCode.A));

            switch (rCommand)
            {
                case SketchCommands.Activate:
                    return Brush.GetCommand(rCommand) || (!isDemoMode && GetMouseButton(0));
                //return API_Module_InputSystem_KS.KSRight.inputDataKS.inputKeys.GetKeyDown(InputKeyCode.RTrigger);
