...
namespace TiltBrush
{
    ...
    public class ViewpointScript : MonoBehaviour
    {
        ...
        //zby增加
        public Transform _controllerRight;
        public GameObject _line;
        public Transform _head;//Head

        private bool _isEnter = false;
        public bool IsMREnter { get { return _isEnter; } }
        private bool _isLong = false;
        public bool IsMRLong { get { return _isLong; } }
        ...
        void Awake()
        {
            ...
            DispatcherBase.KeyDownDelegateRegister(AnyKeyDownEventDelegate);
            DispatcherBase.KeyUpDelegateRegister(AnyKeyUpEventDelegate);
            DispatcherBase.KeyLongDelegateRegister(AnyKeyLongEventDelegate);
        }
        ...
        //zby增加
        void AnyKeyDownEventDelegate(InputKeyCode keyCode, InputDevicePartBase part)
        {
            if (part.inputDeviceBase.inputDeviceType == InputDeviceType.KS && keyCode == InputKeyCode.Enter)
            {
                //DOTO
                _isEnter = true;
                Debug.Log("Down:isenter的值为：" + _isEnter);
            }

            else if (part.inputDeviceBase.inputDeviceType == InputDeviceType.GGT26Dof && keyCode == InputKeyCode.Enter)
            {
                //DOTO
                _isEnter = true;
                Debug.Log("Down:isenter的值为：" + _isEnter);
        ...
        void AnyKeyUpEventDelegate(InputKeyCode keyCode, InputDevicePartBase part)
        {
            if (part.inputDeviceBase.inputDeviceType == InputDeviceType.KS && keyCode == InputKeyCode.Enter)
            {
                //DOTO
                _isEnter = false;
                _isLong = false;
                //Debug.Log("Up:isenter的值为：" + _isEnter);
            }
            else if (part.inputDeviceBase.inputDeviceType == InputDeviceType.GGT26Dof && keyCode == InputKeyCode.Enter)
            {
                //DOTO
            ...
            else if (part == API_GSXR_Module_InputSystem_KS.GSXR_KSRight && InputKeyCode.RFunction == keyCode)
            {
                Application.Quit();
            }
        }
        ...
        void AnyKeyLongEventDelegate(InputKeyCode keyCode, InputDevicePartBase part)
        {
            ...
            if (part.inputDeviceBase.inputDeviceType == InputDeviceType.KS && keyCode == InputKeyCode.Enter)
            {
                ...
                //_isEnter = true;
                _isLong = true;
                Debug.Log("Long:isenter的值为：" + _isLong);
            }
            else if (part.inputDeviceBase.inputDeviceType == InputDeviceType.GGT26Dof && keyCode == InputKeyCode.Enter)
            {
                //DOTO
        ...
        {
            DispatcherBase.KeyDownDelegateUnRegister(AnyKeyDownEventDelegate);
            DispatcherBase.KeyUpDelegateUnRegister(AnyKeyUpEventDelegate);
            DispatcherBase.KeyLongDelegateUnRegister(AnyKeyLongEventDelegate);
        }
