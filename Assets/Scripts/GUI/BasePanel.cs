        ...
        protected void UpdateDescriptions()
        {
            try
            {
                ...
                if (m_PanelDescriptionState != DescriptionState.Closed && m_PanelDescriptionObject != null)
                {
                    ...
                    //zby添加
                    BoxCollider _col = transform.Find("Collider").GetComponent<BoxCollider>();
                    if (_col != null)
                    {
                        m_PanelDescriptionObject.transform.position = _col.transform.position + new Vector3(-_col.size.x/2, _col.size.y/2+0.4f, 0);
                    }
                    else
                    {
                        m_PanelDescriptionObject.transform.position = m_Mesh.transform.position + vTransformedOffset;
