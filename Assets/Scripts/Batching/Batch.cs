...
namespace TiltBrush
{
    ...
    /// A single mesh that contains geometry for multiple strokes.//(包含多个笔划的几何体的单个网格。)
    /// TODO: implement optional attributes//实现可选属性
    public class Batch : MonoBehaviour
    {
        // This must be a multiple of 3//（者必须是3的倍数）
        const int MAX_VERTS_SOFT = 15999;     // The limit above which we try not to go//我们尽量不超过的极限
        const int MAX_VERTS_HARD = 0xfffe;    // This is the Unity limit// 这就是统一极限

        private BatchPool m_ParentPool;

        /// <summary>
        /// batch身上的的MeshFilter组件
        /// </summary>
        ...
        void UpdateMesh()
        {
            ...
            if (m_bVertexDataDirty)
            {
                ...
                m_bVertexDataDirty = false;
                m_bTopologyDirty = false;  // The topology gets updated in CopyToMesh().//在CopyToMesh（）中更新拓扑。
