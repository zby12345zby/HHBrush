...
namespace TiltBrush
{
    ...
    public class BrushCatalog : MonoBehaviour
    {
        ...
        /// ///传递一个GameObject来接收新创建的singleton目录
        /// ///对于单元测试很有用，因为大量倾斜笔刷使用GetBrush（Guid）。
        /// ///TODO:将TB更改为直接使用BrushDescriptor，而不是通过GUID间接使用
        public static void UnitTestSetUp(GameObject container)
        {
            ...
            //对于单元测试，最好是让所有描述符都可用，而不仅仅是清单中的一个子集。
        ...
        /// ///开始重新加载任何来自松散文件的笔刷资源。              ///完成后将触发“BrushCatalogChanged”事件。
        public void BeginReload()
        {
            m_IsLoading = true;

            // Recreate m_GuidToBrush  重新创建mu GuidToBrush
            {
            ...
            // Postprocess: put brushes into parse-friendly list//后置处理：将画笔放入解析友好列表
        ...
        //返回清单的两个部分（compat和non-compat）中的画笔              //仅在compat部分中找到的笔刷将具有m\u HiddenInGui=true
        static private List<Brush> LoadBrushesInManifest()
        {
            ...
            // Additional hidden brushes附加隐藏笔刷
