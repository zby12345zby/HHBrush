...
namespace TiltBrush
{
    ...
    public abstract class QuadStripBrush : BaseBrushScript
    {
        ...
        //初始值不是四元0，就是紧跟在中断之后。
        //              //[初始，前导）是四元半开范围（不是实体！）
        //指向笔划的最后一个无中断部分。这个范围是
        //保证由适当大小的四边形组成。              //
        //也有零或一个固体在领先。该固体（如果存在）
        //将比允许的短。这被称为“前缘”。
        //如果mèu LeadingSegmentInitialQuadIndex！=null，有一个实数。
        /// <summary>
        /// 生成四边形（两个三角形）的数量
        /// </summary>
        ...
        protected override void InitBrush(BrushDescriptor desc, TrTransform localPointerXf)
        {
            ...
            mf.mesh = null;  // Force a new, empty, mf-owned mesh to be generated//强制生成一个新的、空的、mf拥有的网格
            mf.mesh.MarkDynamic();

            // We only need to set verts and tris here because the mesh is zeroed out, effectively hidden
            //我们只需要在这里设置顶点和三角形，因为网格是归零的，有效地隐藏了
        ...
        override public BatchSubset FinalizeBatchedBrush()
        {
            ...
            //“焊接”功能仅支持单面几何图形；但这很好，因为              //不应留下双面四边形刷子。
            if (!m_EnableBackfaces)
            {
                ...
                // writes new indices!
                //把三角形放回原来的样子，因为MasterBrush不希望任何人修改mïTris。我们真的应该摆脱画笔，到处使用几何工具。或者，
                //如果每个QuadStripBrush都通过顶点缩减路径，那么我们就可以留下mèTris垃圾，因为顶点缩减步骤总是写入新的索引！    

                // For MasterBrush, # verts === # indices, since it assumes each tri uses 3 unique verts.
                //对于MasterBrush，#顶点===#索引，因为它假设每个tri使用3个唯一的顶点。
        ...
        /// ///重写顶点和索引，将相同的顶点焊接在一起。
        /// ///输入拓扑必须记录在QuadStripBrush.cs公司
        /// ///输出拓扑与FlatGeometryBrush相同
        ...
        /// Unconditionally increments m_LeadingQuadIndex by 1 or 2无条件地将m\ U LeadingQuadIndex递增1或2
            ...
            out int earliestChangedQuad)
        {
            ...
            // Create duplicates if we have backfaces enabled.//如果启用了背面，则创建重复项。

            //Debug.Log("m_EnableBackfaces的值为：" + m_EnableBackfaces);
            if (m_EnableBackfaces)//运行时：m_EnableBackfaces==false
            {
            ...
            // Walk backward and smooth out previous quads./向后走，把前面的四边形弄平。
            //如果没有断层，m_LeadingQuadIndex=m_InitialQuadIndex
            int iStripLength = m_LeadingQuadIndex;      // In solids在固体中
            int iSegmentLength = m_LeadingQuadIndex - m_InitialQuadIndex;  // In solids在固体中
            ...
            // We don't need to smooth anything if our strip is only 1 quad.//我们不需要平滑任何东西，如果我们的带只有1个四边形
            if (iStripLength > 1)
            {
                // Indexes for later use//供以后使用的索引
                ...
                if (iSegmentLength == 1)
                {
                    ...
                    // Fuse back to mid if it exists and if they used to be fused.//如果它存在，如果它们曾经被熔合，则熔合回中间。
                    ...
                    else if (bGenerateNew && m_LastSegmentLengthSolids == 1)
                    {
                        ...
                        //如果我们有一个比一个四边形长的条带，而这个线段只有一个四边形，那么 
                        //意味着这是一个新领域的开始。如果我们开始一个新的领域 
                        //我们前面的片段只有一个四边形，挤压这个四边形来清理工件。
                ...
                {
                    //Debug.Log("iSegmentLength == 2,执行了");
                    // If we've got a long strip, but this segment is only 2 quads, just fuse.
                    //后面的四边形索引//如果我们有一个很长的条带，但是这个段只有两个四边形，只要保险丝。
                ...
                {
                    //Debug.Log("iSegmentLength >2,执行了");
                    // Set mid quad to the midpoint of back and front quads.//将中间四边形设置为前后四边形的中点。
                    for (int i = 0; i < 6; ++i)
                    {
                    ...
                    //通过连接后四边形的前缘和后缘来修补孔              //中间的边缘，从中间到前面做同样的操作。
                    FuseQuads(aVerts, aNorms, iBackQuadVert, iMidQuadVert, bGenerateNew);
                    FuseQuads(aVerts, aNorms, iMidQuadVert, iFrontQuadVert, bGenerateNew);
                    ...
                    // Make sure the UVs are proper//确保UV正确
        ...
        //添加围绕点vCenter的四边形对称，该点通过指定方向vForward完全定义              //两个向量，从中心指向四边形的两条垂直边。
        ...
        //连接两个相邻的四边形，没有间隙。如果alterBackQuad为true，则前导四元体的后面和后继四元体的前面一起求平均值，
        //否则，前导四元体的后面设置为等于后继四元体的前面。两个相邻四边形之间共享边顶点上的法向量也被平均。
                       ...
                       bool alterBackQuad)
        {
            ...
            //更新法线。注意我们使用Vector3.Slerp来提供一个任意的非零向量，在罕见的情况下，
            //aNorms[ibackquadrvertindex+1]和aNorms[ifrontquadrvertindex]的平均值为零。实际上，这只发生在立方体模具上的预览笔刷上。
        ...
        //对瞬时压力进行采样并返回平滑值。              //要求与m\u lastpawnpose同步更新m\u lastpawnpression。
            ...
            Vector3 vPos, Quaternion ori, float fPressure)
        {
            ...
            var rMasterBrush = m_Geometry;//存储的笔刷信息（线条信息）
            ...
            // Initial drawing.
            //如果没有生成四边形，则此方法不能修改任何有效状态，
            //否则，我们就有可能在播放不匹配的地方出现错误
            //初始图纸。

            float fSmoothedPressure = GetSmoothedPressure(fPressure, vPos);//平滑压力值
            float fSpawnInterval = GetSpawnInterval(fSmoothedPressure);//生成四边形的间隔长度

            Vector3 vFacing = vPos - m_LastSpawnPos;//上一个点指向新点的向量
            float moveLength = vFacing.magnitude;//上一个点到新点的长度
            if (moveLength < kMinimumMoveLengthMeters_PS * App.METERS_TO_UNITS * POINTER_TO_LOCAL)//如果上一个点到新点的长度小于所要求的长度，
                                                                                                  //则跳出方法
            {
            ...
            vFacing /= moveLength;//把向量归一化

            bool bGenerateNewQuad = moveLength >= fSpawnInterval;//如果上一个点到新点的长度大于生成四边形的间隔长度，则返回true
            int iNumQuadsPer = m_EnableBackfaces ? 2 : 1;//开启背面=2否则=1
            // Compute surface basis vectors.//计算曲面基向量。
            Vector3 vRight, vSurfaceNormal;
            {
                // If single-sided, always point the frontside towards the brush. Causes twisting.//如果是单面的，请始终将正面指向刷子。导致扭曲。
                Vector3 vPreferredRight = m_Desc.m_BackIsInvisible//上一个点的右侧向量
                  ? Vector3.Cross(ori * Vector3.forward, vFacing)
                  : m_LastQuadRight.normalized;


                //Debug.Log("m_LastQuadRight的值为：" + m_LastQuadRight);//m_LastQuadRight==(0,0,0)
            ...
            if (!bGenerateNewQuad)
            {
                // If we're not making a new quad, smooth our facing vector.//如果我们不做一个新的四边形，平滑我们的面对向量。
                float fRatio = moveLength / fSpawnInterval;
                vFacing = Vector3.Slerp(m_LastFacing, vFacing, fRatio);
            }

            float pressuredSize = PressuredSize(fSmoothedPressure) - m_LastSizeShrink;//压力尺寸

            Vector3 vQuadCenter = ((vPos + m_LastSpawnPos) * 0.5f);//四边形的中心点
            Vector3 vQuadForward = vFacing * moveLength * 0.5f;//四边形的forward方向
            Vector3 vQuadRight = vRight * pressuredSize * 0.5f;//四边形的右侧向量
            int iPrevInitialIndex = m_InitialQuadIndex;//上一个初始索引
            float sizeShrink = m_LastSizeShrink;//上次尺寸收缩


            // Check to see if we have a sharp bend.//看看有没有急弯。
            bool bIsBreak = false;
            if (DevOptions.I.AllowStripBreak && !m_PreviewMode)//被执行
            {
                int segmentLength = m_LeadingQuadIndex - m_InitialQuadIndex;
                if (segmentLength >= iNumQuadsPer)//被执行
                {
                    ...
                    if (Vector3.Dot(m_LastQuadForward, vQuadCenter - m_LastSpawnPos) <= 0)//如果有急转弯，即vector3.dot（2-1，3-2）<=0,夹角>90°
                    {
                        bIsBreak = true;
                        // Create a break.//创造一个休息。
                    ...
                    else if (bGenerateNewQuad)//被执行
                    {
                        //Debug.Log("我是bGenerateNewQuad，我被执行了");
                        sizeShrink = m_LastSizeShrink - Mathf.Min(m_LastSizeShrink, 1f * moveLength);
                    }
                }
            }

            // Backup our current index and lay down our leading quad.//备份我们当前的索引，并放下我们的前导四边形。
            int iPrevLeadingIndex = m_LeadingQuadIndex;//记录最新四边形的索引
            int earliestQuad;
            ...
            else
            {
                ...
                //m_LeadingSegmentInitialQuadIndex 现在是 m_LeadingQuadIndex 或 m_InitialQuadIndex，
                //这取决于刚刚接触的四边形是不是断裂。
        ...
        // Called when a segment is broken.当一段中断时调用。
        ...
        //   size - brush size
        //每次领导职位变动都会打电话。
        //通过：              //iqad0，iqad1-四边形的半开范围（不是实体！）更新
        //是m\ U PrevQuadIndex，m\ U LeadingQuadIndex              //大小-画笔大小
