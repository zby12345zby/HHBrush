...
namespace TiltBrush
{
    ...
    /// GeometryPool将网格数据存储在方便的C#可访问的数据结构中。
    /// 通常，GeometryPool数据与特定的单元发动机网使用CopyToMesh（mesh）将数据更改推送到其中。
    /// 这可以在提高内存使用率的同时进行有效的更新，这对CPU缓存非常有利。为了控制内存使用，GeometryPool提供了：
    ///  ///   CopyToMesh(mesh)               Copy GeometryPool -> Mesh                  复制几何工具->网格
    ///   MakeGeometryNotResident(mesh)  Replace memory buffers with a pointer to Mesh   用指向网格的指针替换内存缓冲区
    ///   MakeGeometryNotResident(file)  Dump memory buffers to a file                   将内存缓冲区转储到文件
    ///   MakeGeometryPermanentlyNotResident()   Like it sounds
    ///   EnsureGeometryResident()       Reload memory buffers from the Mesh or file     从网格或文件重新加载内存缓冲区
    ///   GeometryPool目前试图使“not resident”和“resident”之间的区别对调用方尽可能透明。
    ///   如果需要，所有公共方法和属性都将调用EnsureGeometryResident（）。这很方便，但意味着调用者可能会在无意中重新加载缓冲区。
    ///   公共字段（如 m_Triangles）不会（不能）自动重新加载；如果几何体不是常驻的，则它们将为空，并且用户必须显式调用EnsureGeometryResident（）。
    public class GeometryPool
    {
        ...
        {
            // The number of elements. Valid values are 0, 2, 3, and 4.//元素的数量。有效值为0、2、3和4。
            public int size;
        ...
        // Plain-old-data storage for m_BackingFileInfo.//m_BackingFileInfo的普通旧数据存储。
        ...
        // All writes must go through the Layout setter//所有写入操作都必须通过布局设置器
        ...
        //此网格包含可用于重新填充缓冲区的数据。
        //它不是属于我们的，而是由网格的所有者来确保
        //当我们引用网格时，它既不会被修改也不会被删除。
