...
namespace TiltBrush
{
    ...
    public abstract class BaseBrushScript : MonoBehaviour
    {
            ...
            BrushDescriptor desc, Color color, float size_PS)
        {
            ...
            // TODO: pass this into InitBrush and do it there//TODO:将这个传递到InitBrush并在那里执行
        ...
        // used for batched strokes. not overridable  用于批量笔划。不可重写
        ...
        /// ///创建笔划时指针的大小。
        /// ///较大的指针创建较大的线，控制点的密度较低。
        /// ///这也可以看作是“指向本地的指针”比例因子
        ...
        /// The setter should only be used during initialization.///setter只能在初始化期间使用。
        ...
        /// Returns an object that implements the Undo animation///返回实现撤消动画的对象
        ...
        /// ///如果生成永久几何体，则返回true。              ///变换应该在笔划的局部坐标中
        ...
        #region To override
        // Passed transform is relative to the stroke//传递的变换是相对于笔划的
        ...
        // Return true if a new solid was created.如果创建了新实体，则返回true。
        ...
        //这个函数是一个健全的检查，以确保我们没有溢出我们分配的顶点缓冲区              //创建新几何图形时。它是在低水平使用作为保障。
        ...
        // Returns brush size in local (ie canvas) space with pressure factored in.//返回本地（即画布）空间中的笔刷大小，并考虑压力因素。
        ...
        // Returns brush size in local space with pressure and variance factored in.//返回局部空间中的笔刷大小，并考虑压力和方差。
        ...
        // Returns v, flipped so it points in the same direction as desired.//返回v，翻转使其指向所需的同一方向。
            ...
            out Vector3 nRight, out Vector3 nNormal)
        {
            ...
            // surface-normal  x  surface-forward  =  surface-right /曲面法线x曲面向前=曲面右
            ...
            //如果 nPointerF 和 nMove 接近平行（abs（dot）~=1），则方向
            //vRight1的性能不稳定。当拉动刷子时会发生这种情况。在这个
            //在这种情况下，刷牙是正常人的好选择。
            Vector3 vRight2 = InDirectionOf(vPreferredR, Vector3.Cross(nPointerU, nMove));

            // Scale this component up the more unstable vRight1 is.如果vRight1越不稳定，请放大此组件。
          ...
          int iVert0, int iVert1)
        {
            ...
            // For each front-facing-quad in the range//对于范围内的每个前向四边形
